...
ForecastLayout.SCROLL_WIDTH = ForecastLayout.COLUMN_WIDTH * ForecastLayout.VISIBLE_COLUMNS;
ForecastLayout.POPUP_WIDTH = ForecastLayout.DAY_COLUMN_WIDTH + ForecastLayout.SCROLL_WIDTH;

function destroyAllChildren(actor) {
...
class OpenWeatherMenuButton extends PanelMenu.Button {
    ...
    _init(extension) {
        ...
        this._panelMenuManager = Main.panel.menuManager ?? Main.panel._menus;
        if (this._panelMenuManager)
            this._panelMenuManager.addMenu(this.menu);
    ...
    stop() {
        ...
        if (this._panelMenuManager?.removeMenu) {
            this._panelMenuManager.removeMenu(this.menu);
            this._panelMenuManager = undefined;
        }
    }
        ...
        this._settingsC = this._settings.connect("changed", () => {
            ...
            if (this.disableForecastChanged()) {
                ...
                if (this._cities.length === 0)
                    this._cities = "43.6534817,-79.3839347>Toronto >0";
                this.showRefreshing();
                if (this._selectCity.menu.isOpen)
                    this._selectCity.setSubmenuShown(false);
                this._currentLocation = this.extractCoord(this._city);
                this.rebuildSelectCityItem();
                this._clearWeatherCache();
                this.initWeatherData();
    ...
    loadConfigInterface() {
        this._settingsInterface = this._extension.getSettings('org.gnome.desktop.interface');
    ...
    rebuildButtonMenu() {
        destroyAllChildren(this._buttonMenu);
        ...
        this._locationButton.connect('clicked', () => {
            this._selectCity.setSubmenuShown(!this._selectCity.menu.isOpen);
        });
        ...
        this._buttonMenu.add_child(this._buttonBox1);
        this._buttonMenu.add_child(this._buttonBox2);
    }
    ...
    recalcLayout() {
        ...
        this._currentWeather.set_width(ForecastLayout.POPUP_WIDTH);

        if (!this._isForecastDisabled && this._currentForecast !== undefined)
            this._currentForecast.set_width(ForecastLayout.POPUP_WIDTH);

        if (!this._isForecastDisabled && this._forecastDays != 0 && this._forecastExpander !== undefined) {
            this._forecastExpander.set_width(ForecastLayout.POPUP_WIDTH);
            this._forecastExpanderBox.set_width(ForecastLayout.POPUP_WIDTH);
            this._daysBox.set_width(ForecastLayout.DAY_COLUMN_WIDTH);
            this._forecastScrollBox.set_width(ForecastLayout.SCROLL_WIDTH);
            this._forecastScrollBox.show();
            this._forecastExpander.setSubmenuShown(true);
        }
        this._buttonBox1.set_width(ForecastLayout.POPUP_WIDTH - this._buttonBox2.get_width());
    }
    ...
    getWeatherIcon(iconname) {
        ...
        if (this._getUseSysIcons && Gtk.IconTheme.get_default().has_icon(iconname)) {
            ...
            return Gio.icon_new_for_string(getExtensionPath(this._extension) + "/media/status/" + iconname + ".svg");
        }
    }
    ...
    rebuildCurrentWeatherUi() {
        destroyAllChildren(this._currentWeather);
        if (!this._isForecastDisabled)
            destroyAllChildren(this._currentForecast);
        ...
        this._currentWeatherLocation = new St.Label({
            text: _('Please wait'),
            y_align: Clutter.ActorAlign.CENTER,
            style_class: 'openweather-current-location'
        });
        ...
        let currentWeatherDataGrid = new Clutter.GridLayout({
            column_spacing: 6,
            row_spacing: 0
        });
        let rb = new St.Widget({
            layout_manager: currentWeatherDataGrid,
            x_align: Clutter.ActorAlign.END,
            style_class: 'openweather-current-databox'
        });

        let currentWeatherDataRow = 0;
        const addCurrentWeatherDataRow = (caption, valueLabel) => {
            let captionLabel = new St.Label({
                text: caption,
                x_align: Clutter.ActorAlign.END,
                y_align: Clutter.ActorAlign.CENTER,
                style_class: 'openweather-current-databox-captions'
            });
            captionLabel.set_height(20);
            valueLabel.x_align = Clutter.ActorAlign.START;
            valueLabel.y_align = Clutter.ActorAlign.CENTER;
            valueLabel.style_class = 'openweather-current-databox-values';
            valueLabel.clutter_text.line_wrap = false;
            valueLabel.set_height(20);
            currentWeatherDataGrid.attach(captionLabel, 0, currentWeatherDataRow, 1, 1);
            currentWeatherDataGrid.attach(valueLabel, 1, currentWeatherDataRow, 1, 1);
            currentWeatherDataRow++;
        };

        addCurrentWeatherDataRow(_('Feels Like:'), this._currentWeatherFeelsLike);
        addCurrentWeatherDataRow(_('Humidity:'), this._currentWeatherHumidity);
        addCurrentWeatherDataRow(_('Pressure:'), this._currentWeatherPressure);
        addCurrentWeatherDataRow(_('Wind:'), this._currentWeatherWind);
        addCurrentWeatherDataRow(_('Gusts:'), this._currentWeatherWindGusts);
        ...
        this._todaysBox.set_width(ForecastLayout.POPUP_WIDTH);
        ...
        for (let i = 0; i < 4; i++) {
            ...
            todaysForecast.Summary.clutter_text.line_wrap = false;

            let fb = new St.BoxLayout({
                vertical: true,
                width: ForecastLayout.COLUMN_WIDTH,
    ...
    scrollForecastBy(delta) {
        let adjustment = this._getForecastHadjustment();
        if (adjustment === null)
            return;
        adjustment.set_value(adjustment.get_value() + delta);
    }

    _getForecastHadjustment() {
        if (this._forecastScrollBox === undefined)
            return null;
        if (this._forecastScrollBox.hscroll?.adjustment)
            return this._forecastScrollBox.hscroll.adjustment;
        if (this._forecastScrollBox.get_hadjustment)
            return this._forecastScrollBox.get_hadjustment();
        return this._forecastScrollBox.hadjustment ?? null;
    }

    rebuildFutureWeatherUi(cnt) {
        if (this._isForecastDisabled || this._forecastDays === 0)
            return;
        destroyAllChildren(this._forecastExpander.menu.box);

        this._forecast = [];
        this._forecastExpanderBox = new St.BoxLayout({
            width: ForecastLayout.POPUP_WIDTH,
            opacity: 150,
        ...
        this._daysBox = new St.BoxLayout({
            vertical: true,
            width: ForecastLayout.DAY_COLUMN_WIDTH,
        ...
        this._forecastScrollBox = new St.ScrollView({
            width: ForecastLayout.SCROLL_WIDTH,
        ...
        for (let i = 0; i < cnt; i++) {
            ...
            for (let j = 0; j < 8; j++) {
                ...
                forecastWeather[j].Summary.clutter_text.line_wrap = false;
    ...
    _onScroll(actor, event) {
        ...
        this.scrollForecastBy(dy * adjustment.get_step_increment());
        return false;
    }
});
...
export default class OpenWeatherExtension extends Extension {
    enable() {
        openWeatherMenu = new OpenWeatherMenuButton(this);
        Main.panel.addToStatusArea('openWeatherMenu', openWeatherMenu);
        openWeatherMenu.checkPositionInPanel();
    }
