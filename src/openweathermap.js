...
function populateForecastUI() {
    return new Promise((resolve, reject) => {
        try {
            ...
            let forecastUiList = this._forecast ?? [];
            let forecastDays = Math.min(this._forecastDays, forecastUiList.length, forecast.length);

            for (let i = 0; i < forecastDays; i++) {
                let forecastUi = forecastUiList[i];
                let forecastData = forecast[i];

                if (forecastUi === undefined || forecastData === undefined)
                    continue;

                for (let j = 0; j < 8; j++) {
                    if (forecastUi[j] === undefined || forecastData[j] === undefined)
                        continue;
...
function loadJsonAsync(url, params) {
    return new Promise((resolve, reject) => {
        ...
        let _userAgent = this._extension.metadata.uuid;
        if (this._extension.metadata.version !== undefined && this._extension.metadata.version.toString().trim() !== '') {
            _userAgent += '/';
            _userAgent += this._extension.metadata.version.toString();
        }
        ...
        _httpSession.send_and_read_async(_message, GLib.PRIORITY_DEFAULT, null, (_httpSession, _message) => {
            ...
            if (_jsonString instanceof Uint8Array) {
                _jsonString = decoder.decode(_jsonString);
            }
            try {
                if (!_jsonString) {
